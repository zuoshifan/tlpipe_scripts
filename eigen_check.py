...
if (~np.isfinite(sky_vis_xx)).all():
    ...
    e2, U2 = la.eigh(otl_vis_xx)
    e3, U3 = la.eigh(sky_vis_xx - src_vis_xx - otl_vis_xx)

    # print e
    plt.figure()
    plt.plot(range(1, 97), e[::-1], 'bo', label=r'$V$')
    plt.plot(range(1, 97), e1[::-1], 'go', label=r'$V_0$')
    plt.plot(range(1, 97), e2[::-1], 'ro', label=r'$S$')
    plt.plot(range(1, 97), e3[::-1], 'co', label=r'$N$')
    # plt.semilogy(range(1, 97), 1+e[::-1], 'bo', label=r'$V$')
    # plt.semilogy(range(1, 97), 1+e1[::-1], 'go', label=r'$V_0$')
    # plt.semilogy(range(1, 97), 1+e2[::-1], 'ro', label=r'$S$')
    # plt.semilogy(range(1, 97), 1+e3[::-1], 'co', label=r'$N$')
    plt.xlim(0, 97)
    yl, yh = plt.ylim()
    plt.ylim(-0.02*yh, yh)
    ...
    plt.figure()
    plt.plot(range(1, 97), np.abs(U[:, -1]), 'b', label=r'$V$')
    ...
    plt.figure()
    plt.plot(range(1, 97), e[-1]**0.5 * np.abs(U[:, -1]), 'b', label=r'$V$')
...
if (~np.isfinite(sky_vis_yy)).all():
    ...
    e2, U2 = la.eigh(otl_vis_yy)
    e3, U3 = la.eigh(sky_vis_yy - src_vis_yy - otl_vis_yy)
