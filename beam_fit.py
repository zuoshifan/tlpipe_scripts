...
if plot and plot_An:
    for pi, pol in enumerate(pols):
        for fi in ifreqs:
            ...
            plt.xlim(0, nt)
            plt.ylim(1, nfeed+1)
            plt.gca().set_xticks([0, 150, 300, 450, 600])
...
for pi, pol in enumerate(pols):
    for fi in ifreqs:
        ...
        center[pi, fi] = np.where(center[pi, fi] < 0, np.nan, center[pi, fi])
        center_mask = np.ma.masked_invalid(center[pi, fi])
        print 'center min:', np.ma.min(center_mask)
        print 'center max:', np.ma.max(center_mask)
        print 'center median:', np.ma.median(center_mask)
